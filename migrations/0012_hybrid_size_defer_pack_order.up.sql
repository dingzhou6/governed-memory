...
-- Slice D: keep RRF fused-41 membership, 0010 source-block remainder, and 0011
-- lexical_rank<=4 heads, but emit rows with octet_length(content)>2048 last so a
-- huge same-clause lex-head decoy cannot token-stop an in-window complete basis
-- under unchanged 12/4/2160. Lexical-only search_current_memories is unchanged.
