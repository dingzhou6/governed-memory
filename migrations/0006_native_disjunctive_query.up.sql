...
-- One active lexical core; only predicate delivery changes.
CREATE FUNCTION search_current_memories_query_core(
  p_tenant_id text,
...
BEGIN
  ...
  RETURN QUERY
  WITH query_plan AS (
    SELECT p_native_query AS query
  ),
...
AS $$
BEGIN
  RETURN QUERY SELECT * FROM public.search_current_memories_query_core(
    p_tenant_id,p_credential_id,p_request_id,p_query,p_max_context_bytes,p_scope_subject_ids,
    public.lexical_query_bounded_question_v1(p_query));
END
$$;

CREATE FUNCTION lexical_query_disjunctive_v3(p_query text)
RETURNS TABLE (native_query tsquery, preparation_status text)
...
EXCEPTION WHEN syntax_error OR invalid_text_representation THEN
  native_query := public.lexical_query_bounded_question_v1(p_query); preparation_status := 'fallback_roundtrip'; RETURN NEXT;
END
$$;

CREATE FUNCTION search_current_memories_disjunctive_v3(
  p_tenant_id text,
  p_credential_id text,
  p_request_id text,
  p_query text,
...
BEGIN
  SELECT * INTO v_prepared FROM public.lexical_query_disjunctive_v3(p_query);
  PERFORM set_config('app.lexical_preparation_status',coalesce(v_prepared.preparation_status,'fallback_bound'),true);
  RETURN QUERY SELECT * FROM public.search_current_memories_query_core(
    p_tenant_id,p_credential_id,p_request_id,p_query,p_max_context_bytes,p_scope_subject_ids,
    v_prepared.native_query);
END
$$;
REVOKE ALL ON FUNCTION search_current_memories_query_core(text,text,text,text,integer,text[],tsquery), lexical_query_disjunctive_v3(text), search_current_memories_disjunctive_v3(text,text,text,text,integer,text[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION search_current_memories_disjunctive_v3(text,text,text,text,integer,text[]) TO agentic_memory_runtime;
COMMIT;
