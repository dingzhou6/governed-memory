...
-- Slice K / han_deferred_span: keep 0017 English emit (0015) and Han
-- unheaded first_ok + next-passage adj. For Han queries, also: invert
-- one-head neighbor (keep sem<=10 or lex>8 with sem present); keep two-head
-- extra_seq=2 lex<=16 in remainder; fail unheaded extra_seq=1 when lex>=16;
-- deferred sem-ok firsts (sem<=10) still promote pass+1 and pass+3; pair
-- extra_seq 2-3 of the single best first_ok unheaded source when that source
-- has no adj. Packs zh12-p03 with zh-q1/zh-q3 without raising Han local_k,
-- 12/4/2160, or stealing EN 4-row unheaded bases. Singleton k=4 heads stay.
...
WITH lexical_all AS MATERIALIZED (
  ...
  SELECT k.*,
    (k.source_has_head AND k.extra_seq=1 AND k.lexical_rank<=16 AND (
       NOT k.is_han_query
       OR k.source_head_count>=2
       OR (k.semantic_rank IS NOT NULL AND (k.semantic_rank<=10 OR k.lexical_rank>8))
     )) AS is_han_neighbor
  FROM combined_keyed k
), combined_neigh_src AS (
  SELECT n.*,
    coalesce(bool_or(n.is_han_neighbor) OVER (
      PARTITION BY n.item_id, n.revision_id, n.source_revision_id, n.extraction_set_id
    ), false) AS source_has_neighbor
  FROM combined_neighbored n
), combined_gated AS (
  SELECT n.*,
    (n.is_han_query AND NOT n.source_has_head AND NOT n.is_huge AND n.phase=0
     AND n.extra_seq=1
     AND (n.lexical_rank IS NULL OR n.lexical_rank<16)
     AND (
       (n.semantic_rank IS NOT NULL AND n.semantic_rank<=10)
       OR (n.lexical_rank IS NOT NULL AND n.lexical_rank<=6)
       OR (n.semantic_rank IS NULL AND n.lexical_rank IS NOT NULL AND n.lexical_rank<=16)
     )) AS han_unheaded_first_ok,
    (substring(n.passage_id from '-p([0-9]+)$'))::int AS pass_num
  FROM combined_neigh_src n
), combined_deferred AS (
  SELECT g.*,
    (g.is_han_query AND NOT g.source_has_head AND NOT g.is_huge AND g.phase=0
     AND g.extra_seq=1
     AND g.semantic_rank IS NOT NULL AND g.semantic_rank<=10
     AND NOT g.han_unheaded_first_ok) AS han_unheaded_sem_deferred
  FROM combined_gated g
), combined_adj AS (
  SELECT d.*,
    coalesce(bool_or(d.han_unheaded_first_ok) OVER (
      PARTITION BY d.item_id, d.revision_id, d.source_revision_id, d.extraction_set_id
    ), false) AS source_keeps_unheaded_first,
    coalesce(bool_or(d.han_unheaded_sem_deferred) OVER (
      PARTITION BY d.item_id, d.revision_id, d.source_revision_id, d.extraction_set_id
    ), false) AS source_sem_deferred,
    min(d.pass_num) FILTER (WHERE d.extra_seq=1 AND NOT d.source_has_head
      AND NOT d.is_huge AND d.phase=0) OVER (
      PARTITION BY d.item_id, d.revision_id, d.source_revision_id, d.extraction_set_id
    ) AS source_unheaded_first_pass
  FROM combined_deferred d
), combined_adj_marked AS (
  SELECT a.*,
    (a.is_han_query AND NOT a.source_has_head AND NOT a.is_huge AND a.phase=0
     AND a.extra_seq IS NOT NULL AND a.extra_seq<>1
     AND a.pass_num IS NOT NULL
     AND a.source_unheaded_first_pass IS NOT NULL
     AND (
       (a.source_keeps_unheaded_first AND a.pass_num=a.source_unheaded_first_pass+1)
       OR (a.source_sem_deferred AND NOT a.source_keeps_unheaded_first
           AND a.pass_num IN (a.source_unheaded_first_pass+1, a.source_unheaded_first_pass+3))
     )
    ) AS han_unheaded_adj
  FROM combined_adj a
), pair_pick AS (
  SELECT item_id, revision_id, source_revision_id, extraction_set_id
  FROM combined_adj_marked
  WHERE han_unheaded_first_ok
  ORDER BY semantic_rank NULLS LAST, lexical_rank NULLS LAST, source_first,
           item_id, revision_id, passage_id NULLS FIRST
  LIMIT 1
), combined_paired AS (
  SELECT a.*,
    coalesce(bool_or(a.han_unheaded_adj) OVER (
      PARTITION BY a.item_id, a.revision_id, a.source_revision_id, a.extraction_set_id
    ), false) AS source_has_adj,
    EXISTS (
      SELECT 1 FROM pair_pick p
      WHERE p.item_id=a.item_id AND p.revision_id=a.revision_id
        AND p.source_revision_id IS NOT DISTINCT FROM a.source_revision_id
        AND p.extraction_set_id IS NOT DISTINCT FROM a.extraction_set_id
    ) AS is_han_pair_source
  FROM combined_adj_marked a
), combined_pair_marked AS (
  SELECT p.*,
    (p.is_han_query AND NOT p.source_has_head AND NOT p.is_huge AND p.phase=0
     AND p.extra_seq IN (2,3)
     AND p.is_han_pair_source
     AND NOT p.source_has_adj
    ) AS han_unheaded_pair
  FROM combined_paired p
), final_ranked AS (
  SELECT k.*,row_number() OVER (ORDER BY k.phase,
    CASE WHEN k.is_huge THEN 4
         WHEN k.is_lex_head THEN 0
         WHEN k.is_han_neighbor THEN 1
         WHEN k.is_han_query AND k.source_has_head AND k.extra_seq=1
              AND k.lexical_rank<=16 AND NOT k.is_han_neighbor THEN 3
         WHEN k.is_han_query AND k.source_head_count>=2 AND k.extra_seq IS NOT NULL
              AND NOT (k.extra_seq<=2 AND k.lexical_rank IS NOT NULL AND k.lexical_rank<=16) THEN 3
         ...
         WHEN k.is_han_query AND NOT k.source_has_head AND k.extra_seq=1
              AND NOT k.han_unheaded_first_ok THEN 3
         ELSE 2 END,
    CASE WHEN k.is_han_query AND NOT k.source_has_head AND k.han_unheaded_first_ok THEN 1
         WHEN k.is_han_query AND k.han_unheaded_adj THEN 1
         WHEN k.is_han_query AND k.han_unheaded_pair THEN 1
         WHEN k.is_han_query AND NOT k.source_has_head AND k.extra_seq IS NOT NULL
         THEN k.extra_seq ELSE 0 END,
    CASE WHEN k.phase=0 THEN k.source_first ELSE k.phase_rank END,
    CASE WHEN k.han_unheaded_adj THEN 1
         WHEN k.han_unheaded_pair THEN k.extra_seq-1
         ELSE 0 END,
