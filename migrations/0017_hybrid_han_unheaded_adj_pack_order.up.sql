...
-- Slice J: keep 0016 English emit (0015) and Han two-head dump RR. For Han
-- queries, also: defer unheaded extra_seq=1 that fail first_ok (sem<=10 OR
-- lex<=6 OR lex-only lex<=16); emit the next passage of a kept unheaded first
-- (han_unheaded_adj, p00→p01 analog) beside that first. Packs zh12-p01 without
-- raising Han local_k, 12/4/2160, or stealing EN 4-row unheaded bases.
-- Singleton k=4 heads stay.
...
WITH lexical_all AS MATERIALIZED (
  ...
  SELECT g.*,
    coalesce(bool_or(g.han_unheaded_first_ok) OVER (
      PARTITION BY g.item_id, g.revision_id, g.source_revision_id, g.extraction_set_id
    ), false) AS source_keeps_unheaded_first,
    min(g.pass_num) FILTER (WHERE g.extra_seq=1 AND NOT g.source_has_head
      AND NOT g.is_huge AND g.phase=0) OVER (
      PARTITION BY g.item_id, g.revision_id, g.source_revision_id, g.extraction_set_id
  ...
  SELECT a.*,
    (a.is_han_query AND NOT a.source_has_head AND NOT a.is_huge AND a.phase=0
     ...
     AND a.source_keeps_unheaded_first
     AND a.pass_num IS NOT NULL
     AND a.source_unheaded_first_pass IS NOT NULL
     AND a.pass_num = a.source_unheaded_first_pass + 1
    ) AS han_unheaded_adj
  FROM combined_adj a
), final_ranked AS (
  SELECT k.*,row_number() OVER (ORDER BY k.phase,
    ...
    CASE WHEN k.han_unheaded_adj THEN 1 ELSE 0 END,
