...
-- Slice E: after the unchanged Han k=4 / fused-41 clause-union window, append at most
-- four authorized hits of the single rarest Han bigram whose collection DF is >= 3.
-- Existing lexical_rank 1..41 stay put (0011 heads / 0012 size-defer unchanged).
-- Dual RRF can then admit a semantic-tail gold (zh-q2 zh15-p01) without raising
-- Han k or flooding extra lex-only rows into the fused 41. English single_bound
-- is unchanged. CREATE OR REPLACE preserves runtime EXECUTE.

CREATE OR REPLACE FUNCTION search_current_memories_clause_union_v1(
  p_tenant_id text,
...
BEGIN
  ...
  han_grams AS (
    SELECT DISTINCT gram[1] AS gram
    FROM public.lexical_clause_queries_v1(p_query) c
    CROSS JOIN LATERAL regexp_matches(c.native_query::text, '''([^'']+)''', 'g') AS gram
    WHERE c.preparation_status = 'han_bigram'
  ),
  gram_df AS (
    SELECT g.gram, count(p.id)::int AS df
    FROM han_grams g
    JOIN complete_active_sets active ON true
    JOIN public.source_passages p
      ON p.tenant_id = active.tenant_id AND p.item_id = active.item_id
     AND p.revision_id = active.revision_id
     AND p.source_revision_id = active.source_revision_id
     AND p.extraction_set_id = active.extraction_set_id
     AND p.search_document @@ quote_literal(g.gram)::tsquery
    GROUP BY g.gram
  ),
  rarest_han_content_gram AS (
    SELECT gram FROM gram_df WHERE df >= 3 ORDER BY df, gram LIMIT 1
  ),
  rare_passage_hits AS (
    SELECT a.item_id, a.revision_id, p.content, a.recorded_at, a.valid_from,
           a.valid_until, a.validity_status, 'passage'::text AS hit_kind,
           p.source_revision_id, p.extraction_set_id, p.id AS passage_id,
           p.locator::text,
           ts_rank(p.search_document, quote_literal(r.gram)::tsquery) AS relevance,
           p.passage_order,
           octet_length(p.content) <= p_max_context_bytes AS fits_global_budget
    FROM authorized_revisions a
    JOIN complete_active_sets active
    ...
    JOIN public.source_passages p
     ...
     AND p.source_revision_id = active.source_revision_id
     AND p.extraction_set_id = active.extraction_set_id
    CROSS JOIN rarest_han_content_gram r
    WHERE p.search_document @@ quote_literal(r.gram)::tsquery
  ),
  rare_limited AS (
    SELECT ranked.*
    FROM (
      ...
      FROM rare_passage_hits h
    ) ranked
    WHERE ranked.local_rank <= 4
  ),
  rare_appended AS (
    SELECT limited.item_id, limited.revision_id, limited.content, limited.recorded_at,
           limited.valid_from, limited.valid_until, limited.validity_status,
           limited.hit_kind, limited.source_revision_id, limited.extraction_set_id,
           limited.passage_id, limited.locator,
           41 + row_number() OVER (ORDER BY limited.passage_id) AS candidate_rank
    FROM rare_limited limited
    WHERE NOT EXISTS (
      SELECT 1 FROM direct_window d
      WHERE d.item_id = limited.item_id AND d.revision_id = limited.revision_id
        AND d.source_revision_id IS NOT DISTINCT FROM limited.source_revision_id
        AND d.extraction_set_id IS NOT DISTINCT FROM limited.extraction_set_id
        AND d.passage_id IS NOT DISTINCT FROM limited.passage_id
    )
  ),
  expansion_spans AS (
  ...
  candidates AS (
    ...
    UNION ALL
    SELECT rare.item_id, rare.revision_id, rare.content, rare.recorded_at,
           rare.valid_from, rare.valid_until, rare.validity_status,
           rare.hit_kind, rare.source_revision_id, rare.extraction_set_id,
           rare.passage_id, rare.locator, NULL::text[] AS parent_passage_ids,
           0 AS phase, rare.candidate_rank
    FROM rare_appended rare
  )
