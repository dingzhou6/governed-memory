...
-- Restore 0014 headed lex-neighbor emit order (no extra_seq>=3 defer).

-- Slice F: keep 0011 k=4 heads, 0012 huge defer, and 0010 source-block remainder,
-- but after heads emit one extra same-source row when that extra still has
-- lexical_rank<=16 (4x the head band). Funès neighbor=1 analog for a headed
-- source; a weaker extra must not steal an unheaded 4-row complete basis.
-- 12/4/2160 unchanged. Lexical-only search_current_memories is unchanged.
...
WITH lexical_all AS MATERIALIZED (
  ...
  SELECT c.*,
    (c.phase=0 AND c.lexical_rank<=4 AND octet_length(c.content)<=2048) AS is_lex_head,
    (c.phase=0 AND octet_length(c.content)>2048) AS is_huge
  FROM combined c
), combined_keyed AS (
  SELECT m.*,
    bool_or(m.is_lex_head) OVER (
