...
-- Slice B: keep RRF fused-41 membership, but emit each source's window rows together
-- in first-appearance order so later same-source in-window IDs pack under 12/4/2160.
-- Lexical-only search_current_memories is unchanged.
...
WITH lexical_all AS MATERIALIZED (
  ...
  SELECT f.item_id,f.revision_id,
       ...
       f.candidate_omitted,0 AS phase,f.direct_rank AS phase_rank,f.source_first
  ...
  SELECT c.item_id,c.revision_id,c.content,c.recorded_at,c.valid_from,c.valid_until,
         ...
         1 AS phase,c.lexical_rank AS phase_rank,c.lexical_rank AS source_first
