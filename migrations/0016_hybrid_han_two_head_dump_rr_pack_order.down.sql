...
-- Slice H: keep 0011 k=4 heads and 0014 extra_seq=1 lex<=16 neighbor, but if
-- that neighbor attached, extra_seq>=3 of the same headed source waits behind
-- the 0010 remainder (unheaded rows and extra_seq=2). Funès analog: head + one
-- neighbor + at most one leftover before yielding. Sources whose first extra
-- missed the neighbor band keep extra_seq>=3 in the remainder (en09-p01).
-- Singleton lex heads stay. 12/4/2160 unchanged.
