...
-- Slice C: keep RRF fused-41 membership and 0010 source-block remainder, but emit
-- in-window lexical_rank<=4 rows first so a later source (source_first<>1) can pack
...
WITH lexical_all AS MATERIALIZED (
  ...
  SELECT c.*,row_number() OVER (ORDER BY c.phase,
    CASE WHEN c.phase=0 AND c.lexical_rank<=4 THEN 0 ELSE 1 END,
